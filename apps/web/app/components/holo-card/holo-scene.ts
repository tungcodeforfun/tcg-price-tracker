...
import { TERMINAL, type FaceText } from "./face-layout";
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    vec3 col = base * mix(1.0, scan, uIntro) + base * glow * 0.6 * uIntro;
...
function faceTexture(text: FaceText, width: number, mode: "color" | "mask", anisotropy: number) {
  const canvas = document.createElement("canvas");
  canvas.width = width;
  canvas.height = Math.round((width * 88) / 63);
  paintFace(canvas, text, mode);
...
async function loadFonts(text: FaceText) {
  const sample = `${text.title}${text.subtitle}${text.price}`;
  const timeout = Promise.withResolvers<void>();
  setTimeout(timeout.resolve, 2500);
  await Promise.race([
    document.fonts.load('700 16px "JetBrains Mono Variable"', sample),
    timeout.promise,
  ]);
}

/** Mounts the 3D card into `stage`, tracking the pointer over `host`. Returns the disposer. */
export function mountHoloScene(
  host: HTMLElement,
  stage: HTMLElement,
  options: HoloSceneOptions,
): () => void {
  const { onReady, onLost } = options;
  ...
  const bodyMaterial = [
    new MeshBasicMaterial({ color: TERMINAL.screen }),
    new MeshBasicMaterial({ color: TERMINAL.edge }),
  ];
  ...
  // Phosphor outline, plus a faint ghost behind the card.
  const edgeGeometry = new EdgesGeometry(bodyGeometry, 25);
  ...
  function fitTextures() {
    const width = Math.min(
      2048,
      Math.max(256, Math.ceil((stage.clientWidth * FACE_FILL * dpr) / 64) * 64),
    );
    if (Math.abs(width - paintedWidth) < width * 0.2) return;
    paintedWidth = width;
    uniforms.uMap.value?.dispose();
    uniforms.uMask.value?.dispose();
    uniforms.uMap.value = faceTexture(text, width, "color", maxAnisotropy);
    uniforms.uMask.value = faceTexture(text, width / 2, "mask", maxAnisotropy);
  }
  ...
  return () => {
    ...
    for (const geometry of [faceGeometry, bodyGeometry, shadowGeometry, edgeGeometry])
      geometry.dispose();
    ...
    for (const texture of [uniforms.uMap.value, uniforms.uMask.value, shadowMap])
      texture?.dispose();
    renderer.dispose();
