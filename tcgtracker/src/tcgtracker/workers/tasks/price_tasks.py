...
def update_card_price(self, card_id: int) -> dict:
    ...
    """
    logger.info("Starting price update for card %s", card_id)
...
async def _update_card_price_async(task: PriceUpdateTask, card_id: int) -> dict:
    ...
    async with db_manager.session() as session:
        ...
        if not card:
            logger.error("Card %s not found", card_id)
        ...
        try:
            ...
            logger.info(
                "Attempting to fetch price from PriceCharting for card %s", card_id
            )
            ...
            if price_data:
                ...
                transformed_data = {
                    "market_price": price_data.get(
                        "complete_price", price_data.get("market_price", 0)
                    ),
                    "low_price": price_data.get(
                        "loose_price", price_data.get("low_price", 0)
                    ),
                    "high_price": price_data.get(
                        "new_price", price_data.get("high_price", 0)
                    ),
                    "mid_price": price_data.get(
                        "market_price", price_data.get("mid_price", 0)
                    ),
                }
                ...
                for key, value in transformed_data.items():
                    if value and (value < 0 or value > 100000):
                        raise PriceValidationException(
                            f"Invalid price value for {key}: ${value}", price=value
                        )

                price_data = transformed_data
                logger.info(
                    "Successfully fetched price from PriceCharting for card %s", card_id
                )

        except RateLimitException as e:
            logger.warning(
                "PriceCharting rate limit exceeded for card %s: %s", card_id, e
            )
            # Don't fallback for rate limits - wait and retry later
            if e.retry_after:
                raise  # Re-raise to trigger Celery retry with delay
        except AuthenticationException as e:
            logger.error("PriceCharting authentication failed: %s", e)
            # Allow fallback for auth failures but log the issue
            logger.warning("Falling back to JustTCG due to PriceCharting auth failure")
        except PriceValidationException as e:
            logger.error(
                "Invalid price data from PriceCharting for card %s: %s", card_id, e
            )
            # Invalid data should trigger fallback
        except NetworkException as e:
            logger.warning(
                "Network issue with PriceCharting for card %s: %s", card_id, e
            )
            # Network issues should trigger fallback
        except Exception as e:
            logger.warning("Unexpected PriceCharting error for card %s: %s", card_id, e)

        # Fallback to JustTCG if PriceCharting fails
        if not price_data:
            try:
                logger.info("Falling back to JustTCG for card %s", card_id)
                ...
                price_data = await task.justtcg_client.get_card_price(
                    card_identifier=card.name if card.name else str(card.id), game=game
                )
                ...
                if price_data:
                    ...
                    logger.info(
                        "Successfully fetched price from JustTCG for card %s", card_id
                    )

            except RateLimitException as e:
                logger.error("JustTCG rate limit exceeded for card %s: %s", card_id, e)
                raise  # Re-raise for Celery retry
            except PriceValidationException as e:
                logger.error(
                    "Invalid price data from JustTCG for card %s: %s", card_id, e
                )
                # Both sources failed with invalid data
            except Exception as e:
                logger.error("JustTCG also failed for card %s: %s", card_id, e)

        if not price_data:
            logger.warning("No price data found from any source for card %s", card_id)
        ...
        try:
            ...
            logger.error("Error updating price for card %s: %s", card_id, str(e))
            raise
...
def cleanup_old_price_history(days_to_keep: int = 90) -> dict:
    ...
    asyncio.set_event_loop(loop)
    try:
        result = loop.run_until_complete(_cleanup_old_price_history_async(days_to_keep))
        return result
    finally:
        loop.close()
...
async def _cleanup_old_price_history_async(days_to_keep: int) -> dict:
    ...
    async with db_manager.session() as session:
        ...
        await session.execute(
            PriceHistory.__table__.delete().where(PriceHistory.timestamp < cutoff_date)
        )
        await session.commit()
