...
from tcgtracker.database.models import Card, PriceHistory, User, UserAlert
...
async def _check_price_alert_async(alert_id: int) -> dict:
    ...
    async with db_manager.session() as session:
        ...
        result = await session.execute(
            select(UserAlert)
            .options(selectinload(UserAlert.card), selectinload(UserAlert.user))
        ...
        if triggered:
            ...
            await _send_alert_notification(alert.user, alert.card, trigger_reason)
...
async def _check_all_price_alerts_async() -> dict:
    ...
    async with db_manager.session() as session:
        ...
        result = await session.execute(select(UserAlert).where(UserAlert.is_active))
...
async def _send_alert_notification(user: User, card: Card, reason: str) -> None:
