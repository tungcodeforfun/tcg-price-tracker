...
class TCGSet(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    tcg_type: Mapped[TCGTypeEnum] = mapped_column(Enum(TCGTypeEnum), nullable=False, index=True)
...
class Card(Base, TimestampMixin):
    ...
    tcg_set_id: Mapped[Optional[int]] = mapped_column(
        Integer, 
        ForeignKey("tcg_sets.id", ondelete="SET NULL"),
        index=True
    )
    
    # Relationships
    tcg_set: Mapped[Optional["TCGSet"]] = relationship("TCGSet", back_populates="cards")
    price_history: Mapped[List["PriceHistory"]] = relationship("PriceHistory", back_populates="card")
    user_alerts: Mapped[List["UserAlert"]] = relationship("UserAlert", back_populates="card")
    
    # Constraints and Indexes
    __table_args__ = (
        UniqueConstraint("tcg_type", "set_identifier", "card_number", name="uq_cards_type_set_number"),
        Index("idx_cards_tcg_set", "tcg_type", "set_identifier"),
        Index("idx_cards_popularity", "search_count", postgresql_using="btree"),
        Index("idx_cards_name_search", "card_name", postgresql_using="gin", postgresql_ops={"card_name": "gin_trgm_ops"}),
    )
...
class PriceHistory(Base):
    ...
    source: Mapped[DataSourceEnum] = mapped_column(Enum(DataSourceEnum), nullable=False, index=True)
    ...
    condition: Mapped[CardConditionEnum] = mapped_column(
        Enum(CardConditionEnum), 
        default=CardConditionEnum.NEAR_MINT, 
        nullable=False
    )
    ...
    # Constraints and Indexes
    __table_args__ = (
        Index("idx_price_history_card_time", "card_id", "timestamp", postgresql_using="btree"),
        Index("idx_price_history_recent", "timestamp", postgresql_where="timestamp > (CURRENT_TIMESTAMP - INTERVAL '30 days')"),
        Index("idx_price_history_card_source_time", "card_id", "source", "timestamp"),
        UniqueConstraint("card_id", "source", "timestamp", "condition", name="uq_price_history_card_source_time_condition"),
    )
...
class UserAlert(Base, TimestampMixin):
    ...
    comparison_operator: Mapped[str] = mapped_column(String(5), nullable=False)  # <=, >=, =, <, >
    ...
    # Indexes
    __table_args__ = (
        Index("idx_user_alerts_active", "card_id", "price_threshold", "comparison_operator", postgresql_where="is_active = true"),
        Index("idx_user_alerts_user_active", "user_id", "is_active"),
    )
    
    def __repr__(self) -> str:
        return f"<UserAlert(id={self.id}, user_id={self.user_id}, card_id={self.card_id})>"
...
class DataSource(Base, TimestampMixin):
    ...
    def __repr__(self) -> str:
        return f"<DataSource(id={self.id}, name='{self.name}', active={self.is_active})>"
