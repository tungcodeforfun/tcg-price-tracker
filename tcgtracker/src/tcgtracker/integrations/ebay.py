...
class eBayClient(BaseAPIClient):
    ...
    async def _get_application_token(self) -> str:
        ...
        try:
            ...
            raise AuthenticationError(f"Failed to get eBay application token: {str(exc)}")
    ...
    async def _ensure_valid_token(self) -> None:
        ...
        async with self._token_lock:
            ...
            self._token_expires_at = datetime.utcnow() + timedelta(hours=2, minutes=-5)  # 5min buffer
    ...
    async def get_items_by_item_group(self, item_group_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Items in the group
        """
        return await self.get(f"/buy/browse/v1/item_summary/get_items_by_item_group", 
    ...
    async def search_and_analyze_card_prices(
        ...
        if tcg_type.lower() == "pokemon":
            items = await self.search_pokemon_cards(card_name, set_name, condition, limit)
        elif tcg_type.lower() in ("onepiece", "one_piece"):
            items = await self.search_one_piece_cards(card_name, set_name, condition, limit)
        else:
