...
class TCGPlayerClient(BaseAPIClient):
    ...
    def __init__(
        ...
        self.client_secret = client_secret or settings.external_apis.tcgplayer_client_secret
    ...
    async def _get_authorization_url(self, redirect_uri: str, state: Optional[str] = None) -> str:
        """
    ...
    async def _exchange_auth_code_for_tokens(self, auth_code: str, redirect_uri: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise AuthenticationError(f"Failed to exchange authorization code: {str(exc)}")
    ...
    async def _make_request(
        ...
        **kwargs,
        ...
        try:
            return await super()._make_request(method, endpoint, params, json, headers, **kwargs)
    ...
    async def get_product_pricing(
        ...
        if len(product_ids) > 250:
            raise ValidationError("Cannot request pricing for more than 250 products at once")
    ...
    async def get_market_prices(
        ...
        """
        if len(product_ids) > 250:
            raise ValidationError("Cannot request market prices for more than 250 products at once")
