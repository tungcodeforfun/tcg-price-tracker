...
class SearchValidator:
    ...
    def validate_limit(cls, limit: Optional[int]) -> int:
        ...
        if not isinstance(limit, int):
            raise DataValidationException("Limit must be an integer", field="limit")

        if limit < 1:
            raise DataValidationException("Limit must be at least 1", field="limit")

        if limit > cls.MAX_LIMIT:
            raise DataValidationException(
                f"Limit cannot exceed {cls.MAX_LIMIT}", field="limit"
            )
