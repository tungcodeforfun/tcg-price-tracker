...
class TesteBayClient:
    ...
    async def test_ensure_valid_token_new_token(self, client, mock_token_response):
        ...
        with patch.object(client, '_get_application_token', new_callable=AsyncMock) as mock_get_token:
    ...
    async def test_prepare_headers(self, client):
        ...
        assert headers["X-EBAY-C-ENDUSERCTX"] == "contextualLocation=country=US,zip=90210"
    ...
    async def test_search_items_limit_validation(self, client):
        ...
        with pytest.raises(ValidationError, match="eBay Browse API limit cannot exceed 200"):
    ...
    async def test_search_pokemon_cards(self, client, mock_search_response):
        ...
        with patch.object(client, 'search_items', new_callable=AsyncMock) as mock_search:
    ...
    @pytest.mark.asyncio
    async def test_search_and_analyze_card_prices_pokemon(self, client, mock_search_response):
        """Test card price analysis for Pokemon."""
        with patch.object(client, 'search_pokemon_cards', new_callable=AsyncMock) as mock_search:
            mock_search.return_value = mock_search_response["itemSummaries"]

            with patch.object(client, 'get_price_statistics', new_callable=AsyncMock) as mock_stats:
                ...
                mock_search.assert_called_once_with("Charizard", "Base Set", "near_mint", 50)
                mock_stats.assert_called_once()

    @pytest.mark.asyncio
    async def test_search_and_analyze_card_prices_onepiece(self, client, mock_search_response):
        """Test card price analysis for One Piece."""
        with patch.object(client, 'search_one_piece_cards', new_callable=AsyncMock) as mock_search:
            mock_search.return_value = mock_search_response["itemSummaries"]
    ...
    async def test_authentication_error_handling(self, client):
        ...
        with patch.object(client, '_get_application_token', new_callable=AsyncMock) as mock_get_token:
            ...
            with pytest.raises(AuthenticationError, match="Failed to get eBay application token"):
                await client._ensure_valid_token()
