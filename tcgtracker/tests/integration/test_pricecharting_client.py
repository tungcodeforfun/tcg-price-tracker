...
class TestPriceChartingClient:
    ...
    async def test_get_pokemon_products(self, client):
        ...
        with patch.object(client, 'get_pokemon_products', new_callable=AsyncMock) as mock_search:
    ...
    async def test_get_one_piece_products(self, client):
        ...
        with patch.object(client, 'get_one_piece_products', new_callable=AsyncMock) as mock_search:
            mock_search.return_value = [
                {
                    "id": "op-1",
                    "name": "Monkey D. Luffy",
    ...
    async def test_price_transformation(self, client):
        ...
        assert transformed["market_price"] == Decimal("23.75")  # Average of loose, complete, new
    
    @pytest.mark.asyncio
    async def test_fallback_to_justtcg(self):
        """Test fallback mechanism when PriceCharting fails."""
        with patch('tcgtracker.workers.tasks.price_tasks.PriceUpdateTask.pricecharting_client') as mock_pc:
            with patch('tcgtracker.workers.tasks.price_tasks.PriceUpdateTask.justtcg_client') as mock_jt:
                # Mock PriceCharting to fail
                mock_pc.get_card_price = AsyncMock(side_effect=Exception("API Error"))
                
                # Mock JustTCG to succeed
                mock_jt.get_card_price = AsyncMock(return_value={
                    "market_price": 15.00,
                    "low_price": 10.00,
                    "high_price": 20.00,
                    "mid_price": 15.00,
...
class TestPriceUpdateIntegration:
    ...
    async def test_price_update_uses_pricecharting(self):
        ...
        with patch('tcgtracker.workers.tasks.price_tasks._update_card_price_async') as mock_update:
